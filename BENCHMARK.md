...
Domaine **kyc**, difficulté **medium**, seed **42**.  
Mesures via `dupehell` Python wheel (PyO3, single-thread).

## Métriques

| Taille | Records générés | Temps | rec/s | exact_dups | hard_negs | masters |
|---|---|---|---|---|---|---|
...
- **Throughput max** : 836 K rec/s à 25M, puis plateau à ~630 K rec/s pour 75M
- **Goulot d'étranglement** : `sink` (génération + écriture IPC) représente ~80% du temps ; `gt` (ground truth) ~15% ; `alloc` (IDs) ~5%
- **10M en 17s**, **50M en 75s**, **75M en 2 min**, **100M en ~3 min** — scaling quasi-linéaire
