...
def generate(
    ...
    Args:
        ...
        hard_neg_ratio: Internal scaling knob for the hard-negative count, **not**
            ...
            count is approximately ``n_duplicates * hard_neg_ratio * 0.125``,
            where ``n_duplicates`` depends on ``difficulty`` (light ~0.28,
            medium ~0.40, hell ~0.57, as a fraction of ``size``); at
            ``difficulty="medium"`` with the default ``0.3`` this gives ~1.5%
            of ``size``. Use :func:`estimate_difficulty` with the same value
            to see the exact count before generating.
...
def estimate_difficulty(
    ...
    Args:
        ...
        hard_neg_ratio: Same scaling knob as in :func:`generate` (actual count
            is approximately ``n_duplicates * hard_neg_ratio * 0.125``, see
            :func:`generate`) — must match the value used for actual
            generation for the estimate to be meaningful.
